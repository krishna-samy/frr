...
def build_topo(tgen):
    ...
    # Create 515 connections between r1 and r2
    for switch in range(1, 516):
        switch = tgen.add_switch("sw{}".format(switch))
        switch.add_link(r1)
        switch.add_link(r2)
    
    # Create 1 connection between r2 and r3
    switch = tgen.add_switch("sw516")
    switch.add_link(r2)
    switch.add_link(r3)
...
def test_nothing():
    "Do Nothing"
    tgen = get_topogen()
    step("krishna: Sending 10000 sharp routes from r1")
    r1 = tgen.gears["r1"]
    r2 = tgen.gears["r2"]
    r1.vtysh_cmd("sharp install route 1001:111::1 nexthop 2001:db8::1 10000")
    step("krishna: Stopping r1")
    stop_router(tgen, "r1")
    step("krishna: Starting r1")
