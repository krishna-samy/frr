...
def test_scale_interface_flap_nhe_parity():
	"""
	Scale test: install 500k SHARP routes from r2, flap first interface on r2,
	...
	# Install 500k routes starting 45.1.1.1 from r2
	r2.vtysh_cmd("sharp install routes 45.1.1.1 nexthop 10.0.1.2 500000")
	...
	def _converged():
		...
		return isinstance(total_fib, int) and total_fib >= 500000

	ok, _ = topotest.run_and_expect(_converged, True, count=60, wait=1)
	assert ok, "Convergence timeout: routesTotalFib did not reach >= 500,000"
	...
	def _parity_ok():
		...
		# Expect 500001: 500k SHARP-installed prefixes plus 1 route from r3
		return (ing_ref == 500001) and (rng_ref == 500001)
	...
	r2.vtysh_cmd("sharp remove routes 45.1.1.1 500000")
	tsleep(2)
