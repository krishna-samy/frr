...
#
# test_bgp_ecmp_8way_krishna.py
#
# Copyright (c) 2025 by
# Test for BGP ECMP topology with 8 ways
#

"""
test_bgp_ecmp_8way_krishna.py: Testing BGP 8-way ECMP setup
                      - 2 routers connected by 8 links
                      - r1 and r2: all links in default VRF
                      - 8 eBGP sessions (1 per link) for ECMP
...
def build_topo(tgen):
    """
    Build topology with 2 routers connected by 8 links
    Both r1 and r2 have all links in default VRF for 8-way ECMP testing
    """
    tgen.add_router("r1")
    tgen.add_router("r2")
    ...
    # Create 8 links between r1 and r2
    for i in range(1, 9):
...
def test_topology_setup():
    ...
    logger.info("Topology created successfully with 8-way ECMP setup")
    ...
    logger.info("BGP 8-way ECMP topology is ready for testing")


def test_bgp_sessions_established():
    """Test that all 8 BGP sessions are established"""
    tgen = get_topogen()
    # Don't run this test if we have any failure.
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)
    ...
    def check_r1_sessions(router):
        ...
        try:
            ...
            msg = "r1 total established sessions: {}/8".format(established_count)
            logger.info(msg)
            return established_count == 8  # Expect all 8 sessions
    ...
    if not result:
        ...
        logger.info("SUCCESS: All 8 BGP sessions established on r1")
    ...
    def check_r2_sessions(router):
        ...
        try:
            ...
            msg = "r2 total established sessions: {}/8".format(established_count)
            logger.info(msg)
            return established_count == 8  # Expect all 8 sessions
        except Exception as e:
    ...
    if not result:
        ...
        logger.info("SUCCESS: All 8 BGP sessions established on r2")
...
def test_interface_verification():
    ...
    # Check that both routers have 8 interfaces
    for router, router_name in [(r1, "r1"), (r2, "r2")]:
        ...
        expected_msg = "{} should have 8 interfaces, found {}".format(
            router_name, interface_count)
        assert interface_count == 8, expected_msg
