...
static inline char *bgp_route_dump_path_info_flags(struct bgp_path_info *pi, char *buf, size_t len)
{
...
static struct bgp_dest *bgp_path_info_reap_unsorted(struct bgp_dest *dest, struct bgp_path_info *pi)
{
	bgp_path_info_mpath_dequeue(pi);
	...
	*paths_eq = 0;
	...
	if (debug) {
		...
		bgp_path_info_path_with_addpath_rx_str(bpi_ultimate, exist_buf,
						       sizeof(exist_buf));
		zlog_debug("%s(%s): Comparing %s flags %s with %s flags %s", pfx_buf,
			   bgp->name_pretty, new_buf,
			   bgp_route_dump_path_info_flags(new, buf1, sizeof(buf1)), exist_buf,
			   bgp_route_dump_path_info_flags(exist, buf2, sizeof(buf2)));
	}
			...
			safi_t safi)
{
	...
	if (!old_select) {
		...
		if (old_select && !CHECK_FLAG(old_select->flags, BGP_PATH_SELECTED))
			old_select = NULL;
	}
	...
	while (unsorted_list) {
		...
		if (BGP_PATH_HOLDDOWN(first)) {
			 ...
			 */
			if (debug)
				zlog_debug("%s: %pBD(%s) pi %p from %s in holddown", __func__, dest,
					   bgp->name_pretty, first, first->peer->host);

			if (old_select != first && CHECK_FLAG(first->flags, BGP_PATH_REMOVED)) {
		...
		for (look_thru = bgp_dest_get_bgp_path_info(dest); look_thru;
			...
			if (BGP_PATH_HOLDDOWN(look_thru)) {
				...
				if (debug)
					zlog_debug("%s: %pBD(%s) pi from %s %p in holddown",
						   ...
						   look_thru->peer->host, look_thru);

				if (CHECK_FLAG(look_thru->flags, BGP_PATH_REMOVED) &&
				    (look_thru != old_select)) {
					dest = bgp_path_info_reap(dest, look_thru);
					assert(dest);
				}
			...
			if (look_thru->peer && look_thru->peer != bgp->peer_self &&
			    !CHECK_FLAG(look_thru->peer->sflags, PEER_STATUS_NSF_WAIT))
				if (!peer_established(look_thru->peer->connection)) {
					if (debug)
			...
			if (CHECK_FLAG(bgp->flags, BGP_FLAG_DETERMINISTIC_MED) &&
			    (!CHECK_FLAG(look_thru->flags, BGP_PATH_DMED_SELECTED))) {
				bgp_path_info_unset_flag(dest, look_thru, BGP_PATH_DMED_CHECK);
				if (debug)
					zlog_debug("%s: %pBD(%s) pi %s dmed", __func__, dest,
						   bgp->name_pretty, look_thru->peer->host);
			...
			if (bgp_path_info_cmp(bgp, first, look_thru, &paths_eq, mpath_cfg, debug,
					      pfx_buf, afi, safi, &reason)) {
				first->reason = reason;
				worse = look_thru;
				/*
		...
		if (!worse) {
			struct bgp_path_info *end = bgp_dest_get_bgp_path_info(dest);
				 ...
				 afi_t afi, safi_t safi)
{
	...
	if (new_select) {
		if (debug)
			zlog_debug("%s: %pBD setting SELECTED flag", __func__, dest);
